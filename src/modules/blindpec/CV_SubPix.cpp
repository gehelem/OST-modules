                       ...
                       )
{
...

    {
        ...
        */
        ...
        Size MaxScan;
        MaxScan.width = HeatMap.cols >> 4;
        MaxScan.height = HeatMap.rows >> 4;
        ...
        Point ScanRectMax;
        ScanRectMin.x = LocIn->x - MaxScan.width;
        if (ScanRectMin.x  < 0) ScanRectMin.x = 0;
        ScanRectMin.y = LocIn->y - MaxScan.height;
        if (ScanRectMin.y  < 0) ScanRectMin.y = 0;
        ScanRectMax.x = LocIn->x + MaxScan.width;
        if (ScanRectMax.x  >=  HeatMap.cols ) ScanRectMax.x = HeatMap.cols - 1;
        ScanRectMax.y = LocIn->y + MaxScan.height;
        if (ScanRectMax.y  >=  HeatMap.rows ) ScanRectMax.y = HeatMap.rows - 1;
...
void SubPixFitParabola(cv::Point2d* Result,  cv::Point2d &P1, cv::Point2d &P2, cv::Point2d &P3)
{
    ...
    */
    ...
    double C = (P2.x * P3.x * (P2.x - P3.x) * P1.y + P3.x * P1.x * (P3.x - P1.x) * P2.y + P1.x * P2.x *
                (P1.x - P2.x) * P3.y) / denom;
